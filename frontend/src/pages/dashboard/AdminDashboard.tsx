...
export default function AdminDashboard() {
  ...
  const stats = [
    ...
    { label: 'Registros Hoy', value: '+4', subtitle: 'Últimas 24 horas', icon: TrendingUp, color: 'text-[#D4826A]', bg: 'bg-[#D4826A]/10' },
    ...
    { label: 'Validaciones', value: '3', subtitle: 'Perfiles por revisar', icon: ShieldCheck, color: 'text-[#8B9A6B]', bg: 'bg-[#8B9A6B]/10' },
  ];
        ...
        <div className="lg:col-span-9 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((stat, i) => (
            <motion.div
              ...
              className="bg-white rounded-3xl p-5 border border-gray-100 shadow-sm flex flex-col justify-between hover:shadow-md transition-all group text-left"
            >
        ...
        <div className="bg-white rounded-[2rem] p-6 space-y-6 border border-gray-100 shadow-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <h2 className="text-base font-black text-[#1A1A1A]">Validaciones Pendientes</h2>
                <p className="text-[10px] text-gray-400 font-medium">Revisión de perfiles de empresas y talentos</p>
              </div>
            </div>
            <span className="bg-[#EDE8DB] text-[#6B6B6B] rounded-full px-3 py-1 text-[9px] font-bold">3 pendientes</span>
          </div>
          
          <div className="space-y-3">
            {[1, 2].map((i) => (
              <div key={i} className="flex items-center justify-between p-4 bg-[#F5F0E8]/30 rounded-2xl border border-gray-100 hover:bg-white hover:shadow-lg transition-all group cursor-pointer">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <p className="text-xs font-black text-[#1A1A1A]">Usuario Registrado {i}</p>
                    <p className="text-[9px] text-gray-400 uppercase font-bold tracking-widest mt-0.5">Profesional +45</p>
                  </div>
                </div>
                <button className="px-3 py-1.5 text-[9px] font-black bg-white border border-gray-200 text-[#1A1A1A] uppercase tracking-widest rounded-lg hover:bg-[#2C2C2C] hover:text-white transition-all shadow-sm">Validar</button>
              </div>
            ))}
          </div>
        </div>
